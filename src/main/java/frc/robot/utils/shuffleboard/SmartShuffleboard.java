 ...
 * Adding and updating a value is done in the same manner, and there's no need to handle
 * networkTable entries. Data can be displayed in its own widget or items can be grouped into a list
 * widget. Example:
 *
 * <p>tab name list name field name value SmartShuffleboard.put("Drive", "encoders", "Front Right",
 * steerFR.getSelectedSensorPosition(0)); SmartShuffleboard.put("Drive", "encoders", "Front Left",
 * steerFL.getSelectedSensorPosition(0)); SmartShuffleboard.put("Drive", "encoders", "Rear Right",
 * steerRR.getSelectedSensorPosition(0)); SmartShuffleboard.put("Drive", "encoders", "Rear Left",
 * steerRL.getSelectedSensorPosition(0));
 *
 * <p>tab name field name value SmartShuffleboard.put("Drive", "Gyro", getGyro());
 */
public class SmartShuffleboard {

  private static final Map<String, SmartShuffleboardTab> smartTabMap = new HashMap<>();
  ...
  public static SimpleWidget put(
      String tabName, String layoutName, String fieldName, Object value) {
    SmartShuffleboardTab smartTab = getOrCreateTab(tabName);
